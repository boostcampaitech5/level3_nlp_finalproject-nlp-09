  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios({
        method: "post",
        url: `http://${process.env.REACT_APP_SERVER_URL}/upload_link`,
        data: formData,
            ...
            <form onSubmit={handleSubmit} className='flex flex-row gap-2 itmes-center'>
              ...
              {isFileUpload ? null :
                <div className="flex flex-row justify-center" >
                  <input type="text" onChange={onChange} value={text} placeholder="Put your audio link" className="flex w-full gap-2 bg-white-600 hover:bg-white-800 relative text-black py-2.5 px-20 sm:px-70 rounded border-3 border-slate-500 text-center" style={{ width: 600, paddingLeft: 10, paddingRight: 10 }} />
