...
const History = ({ isActive, id, history, onClickHistory }) => {
  ...
  const handleSave = () => {
    ...
    axios.post(`http://${process.env.BACKEND_SERVER_ADDRESS}/history/change_title`, body).then((res) => {
  ...
  const onClickDel = () => {
    ...
    axios.post(`http://${process.env.BACKEND_SERVER_ADDRESS}/history/delete`, body).then((res) => {
      console.log(res.data);
      const result = res.data
