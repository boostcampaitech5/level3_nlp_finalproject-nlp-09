...
const RETRY_DELAY_MS = 2000; // 2珥? 媛꾧꺽?쑝濡? ?옱?떆?룄
...
export function Ques({ historyId }) {
  ...
  useEffect(() => {
    // transcription?씠 蹂?寃쎈맆 ?븣留덈떎 ?긽?쐞 div ?겕湲? ?뾽?뜲?씠?듃
    updateParentSize();
  }, [qnaList]);
  ...
  return (
    <div id="parentDiv" >
      <div id="parentDiv" className="overflow-y-scroll" style={{
        width: "90%", height: "700px", backgroundColor: 'white', margin: "10px auto", textAlign: "left", paddingTop: "30px", borderRadius: "10px", boxShadow: "5px 5px 10px"
      }}>
        <p class="font-extrabold text-qa text-xl"><em></em></p>

        {qnaList ?
          <div id="childDiv" style={{ height: "325px", fontFamily: "Noto Sans KR", fontWeight: "700", padding: "30px", borderWidth: "3px 0px 3px 0px", borderColor: "black", fontSize: "20px" }}>
            {updatedQNAList.map((qna, index) => (
              <div key={qna.qna_id}>
                {editingItemId === qna.qna_id ?
                  (
                    <div >
                      <div className="flex flex-row gap-2" style={{ fontFamily: "Noto Sans KR", textAlign: "left", color: "#3E6DE2" }}>
                        <div>Q.</div>
                        <textarea
                          value={qna.question}
                    ...
                    <ol key={qna.qna_id} className="flex flex-col w-full transition-colors py-3 px-3 items-start gap-3 relative rounded-md cursor-pointer break-all pr-[4.5rem] )} )} hover:bg-gray-200 group animate-flash" style={{ borderWidth: "3px 0px 3px 0px", borderColor: "black", borderStyle: "solid", borderRadius: "0px", marginBottom: "-3px" }}>
                      <li style={{ fontFamily: "Noto Sans KR", fontWeight: "700", fontSize: "20px", textAlign: "left", marginBottom: "5px", paddingLeft: "4px" }}> Q. {qna.question}</li>
                      ...
                      <div class="flex right-1 z-10 text-gray-300 visible">
                        <button onClick={() => handleEdit(qna.qna_id)} class="p-1 hover:text-white"><svg stroke="black" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg" ><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></button>
                        <button onClick={() => onClickDel(qna.qna_id)} className="p-1 hover:text-white">
                          <svg
                            stroke="black"
                            fill="none"
                            strokeWidth={2}
