...
class SentenceGrouper:
    ...
    def grouping(self, sentences, length_threshold=30, similarity_threshold_for_short=0.2, similarity_threshold_for_long=0.3, length_threshold_for_paragraph=50):
        stack = []
        print("[Sentence-RoBERTa Paragraph Separation]")
...
class TextSummarizer:
    def __init__(self, model_name="traintogpb/pko-t5-large-kor-for-colloquial-summarization-finetuned"):
        self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        assert self.device != 'cuda', f"CUDA GPU is not available now (DEVICE: {self.device})"
    ...
    def inference_group(self, data):
        ...
        for i in data:
            ...
            summary_text = self.tokenizer.decode(
                summary_tokens[0], skip_special_tokens=True)
...
def summarize(transcription):
    ...
    summary_list = []
    print("[PKO-T5 Summarization]")
    for paragraph in tqdm(regrouped_data):
        raw_summary = text_summarizer.inference_group(paragraph)
        summary_list.append(' '.join(raw_summary))
